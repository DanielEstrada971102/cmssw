  ...
  if (nHits != 0) {
    if (debug_) {
      ...
      for (auto &cand : showerCands) {
        showerb::log_debug("ShowerBuilder") << "        - Candidate vector for SL" << cand.first 
                                          << " initialized with size " << cand.second.size();
      }
    }
...
void ShowerBuilder::clear() {
  ...
  showerCands = {
    {1, ShowerCandidatePtrs{}},
    {3, ShowerCandidatePtrs{}},
    {2, ShowerCandidatePtrs{}}
  };
}
...
void ShowerBuilder::processHits_standAlone() {
  ...
  for (int sl : {1, 2, 3}) {
    if (triggerShower(*bmtl1_buffers[sl])) {
      if (debug_)
        showerb::log_debug("ShowerBuilder") << "        o Shower found in SL" << sl << " with " << bmtl1_buffers[sl]->size() << " hits";
      candidates[sl]->flag();
...
void ShowerBuilder::processHitsFirmwareEmulation() {
  ...
  std::map<int, bool> shower_already_set = {
    {1, false},
    {2, false},
    {3, false}
  };
  ...
  for (int bx = min_bx; bx <= max_bx + 17; bx++) {
    ...
    for (int sl : {1, 2, 3}) {
      ...
      if (triggerShower(*bmtl1_buffers[sl]) && !cand->isFlagged()) {
        if (debug_)
          showerb::log_debug("ShowerBuilder") << "        o Shower found in SL" << sl << " with " << nHits[sl] << " hits";
        cand->flag();
      }
...
void ShowerBuilder::fill_bmtl1_buffers(const int bx) {
  ...
  for (const auto& pair : obdt_buffers) {
    ...
    while (!obdt_buffer.empty()) { // if empty, nothing to do
      ...
      (*hits_count[sl])++;
      if (dump_digis_) dump_digi_to_tree(_hitpbx);
...
void ShowerBuilder::bxStep(const int _current_bx) {
  ...
  std::map<int, int> obdt_hits_bxpersistences = {
    {1, obdt_hits_bxpersistence_phi_},
    {3, obdt_hits_bxpersistence_phi_},
    {2, obdt_hits_bxpersistence_z_}
  };
  for (auto& pair : obdt_buffers) {
    int sl = pair.first;
