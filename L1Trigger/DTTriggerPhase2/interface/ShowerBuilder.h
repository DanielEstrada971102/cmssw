...
namespace showerb {
  ...
  inline void set_wire_properties(ShowerCandidatePtr & shower_cand, DTPrimitives& hits) {
    ...
    wires_profile.clear();
    wires_constituents.clear();
    wires_layer_constituents.clear();
    wires_tdc_constituents.clear();
    for (auto& hit : hits) {
  ...
  inline bool buffer_contains(const ShowerBuffer& buffer, const DTPrimitive& hit) {
    for (const auto& item : buffer) {
      if (item.second.channelId() == hit.channelId() && 
        item.second.layerId() == hit.layerId() &&
  ...
  inline std::string buffer_to_string(const ShowerBuffer& buffer) {
    ...
    for (const auto& hit : buffer) {
      if (contents.length() > 1) contents += ", ";
      contents += std::to_string(hit.first) + ":" + 
                  std::to_string(hit.second.superLayerId()) + ":" + 
                  std::to_string(hit.second.layerId()) + ":" + 
                  std::to_string(hit.second.channelId());
    }
    contents += "]";
    return contents;
  }
  ...
  inline DebugLogger log_debug(const std::string& category) {
    return DebugLogger(category);
  }
}  // namespace showerb
...
public:
  ...
  ShowerBuilder(const edm::ParameterSet& pset, edm::ConsumesCollector& iC, TTree* tree);  // Changed from std::shared_ptr<TTree> to TTree*
