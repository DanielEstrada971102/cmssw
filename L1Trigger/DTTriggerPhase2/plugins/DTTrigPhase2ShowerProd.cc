...
DTTrigPhase2ShowerProd::DTTrigPhase2ShowerProd(const ParameterSet& pset) 
  ...
  showerBuilder = std::make_unique<ShowerBuilder>(
    pset, 
    consumesColl, 
    dump_digis_ ? m_dumped_digis_tree : nullptr
  );
...
void DTTrigPhase2ShowerProd::produce(edm::Event& iEvent, const edm::EventSetup& iEventSetup) {
  ...
  if (debug_)
    showerb::log_debug("DTTrigPhase2ShowerProd") << "    Hits preprocessed: " << digiMap.size() 
                                                  << " DT chambers to analyze";
  ...
  if (dump_digis_) m_dumped_digis_tree->Fill();

  // 3. Check shower candidates and store them if flagged
  if (debug_)
    showerb::log_debug("DTTrigPhase2ShowerProd") << "    Selecting shower candidates (to check: " 
                                                  << showersMap.size() << " )";
  ...
  for (auto& sl_showerCandIt : showersMap) {
    ...
    for (auto& showerCandPtr : showerCandPtrs){
      if (showerCandPtr->isFlagged()) {
        ...
        if (debug_)
          showerb::log_debug("DTTrigPhase2ShowerProd") << "      Shower candidate tagged in chamber" 
